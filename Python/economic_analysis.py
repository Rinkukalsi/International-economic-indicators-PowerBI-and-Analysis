...
!pip install xlsxwriter # Import the xlsxwriter library for Excel export functionality
...
## Exporting Data and Charts to Excel

#To export your summary table and the generated charts into a single Excel file, you can define a function that utilizes `pd.ExcelWriter` with the `xlsxwriter` engine. This allows you to write multiple dataframes to different sheets and also embed images.

#```python
def export_to_excel(summary_df, gdp_chart_path, inflation_chart_path, output_filename="Economic_Summary.xlsx"):
    """
    Exports the summary table and chart images to an Excel file.

    Args:
        summary_df (pd.DataFrame): The summary table to be exported.
        gdp_chart_path (str): The file path of the saved GDP growth chart image.
        inflation_chart_path (str): The file path of the saved 2024 inflation chart image.
        output_filename (str): The desired name for the output Excel file.
    """
    with pd.ExcelWriter(output_filename, engine='xlsxwriter') as writer:
        # Write the summary table to a sheet
        summary_df.to_excel(writer, sheet_name='Summary Data', index=False)

        # Get the xlsxwriter workbook and worksheet objects
        workbook = writer.book
        worksheet_summary = writer.sheets['Summary Data']

        # Add the GDP growth chart image
        # You can adjust the cell where the image is inserted (e.g., 'A10')
        worksheet_summary.insert_image('A' + str(len(summary_df) + 3), gdp_chart_path)

        # Add the 2024 inflation chart image
        # Adjust the starting row to place it below the first chart or in a new column
        worksheet_summary.insert_image('A' + str(len(summary_df) + 3 + 20), inflation_chart_path) # Adjust row offset as needed

    print(f"\nSuccessfully exported data and charts to '{output_filename}'")

# Call the function to export
export_to_excel(summary_table, gdp_chart_filename, inflation_chart_filename, "Economic_Report.xlsx")
